...
d3.json(industry_url_0).then(function(info) {
    //console.log(info);
    buildBubbleChart(info,"Revenue(10millions)","Growth(%)","location_card","1000","12","States Overview- Average Growth vs. Revenue");
...
function buildBarChart(data,name1,name2,chartId,bar_type,bar_width,f_size,title_info,description){
  ...
  var layout = {
    title: title_info +`Growth vs. Revenue :Total ${num} company(ies)`+ description,
    xaxis: {
      tickangle: -40
    },
    barmode: bar_type,
...
function optionChanged_industry(industry){
        ...
        d3.json(industry_filter_url).then(function(info) {
            ...
            var title_info = `States Overview in ${industry} Sector:`;
            buildBarChart(plot_list,"Growth(%)","Revenue(10k)","location_card","stack","950","8",title_info,`in ${industry}`);
...
function optionChanged_years(year) {
    ...
    d3.json(years_filter_url).then(function(info) {
        ...
        var title_info = `States Overview in ${year}-Year on the List:`;
            
        buildBarChart(plot_list,"Growth(%)","Revenue(10k)","location_card","stack","950","8",title_info,`  stayed in ${year}-year on the list.`);
...
function optionChanged_founded(founded){
    ...
    d3.json(founded_filter_url).then(function(info) {
        ...
        var info_list =[];
        info.forEach(item =>{
            info_list.push([item["City"],item["Growth"],item["Revenue"]/(1000000)]);
        ...
        var title_info = `City Overview in companies founded ${founded}:`;
            
        buildBarChart(plot_list,"Growth(%)","Revenue(M)","location_card","stack","950","8",title_info,` founded in ${founded}.`);
...
function optionChanged_state(state_l){
    ...
    d3.json(state_filter_url).then(function(info) {
        ...
        var title_info =`${state_l} Overview in Industry Sector:`;
        
        buildBarChart(plot_list,"Growth(%)","Revenue(10k)","location_card","stack","950","8",title_info,` in ${state_l}.`);
...
function optionChanged_city(city){
    ...
    d3.json(city_filter_url).then(function(info) {
        ...
        var title_info =`${city} Overview in Industry Sector:`;
        buildBarChart(plot_list,"Growth(%)","Revenue(10k)","location_card","stack","950","8",title_info,` in ${city}.`);
